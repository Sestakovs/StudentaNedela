# Viena darba nedēļa Datorikas nodaļas studenta dzīvē

Statiska frontend vietne GitHub Pages publicēšanai. Projektā ir:

- viena HTML lapa ar sākuma sadaļu un piecām tematiskām sadaļām;
- latviešu teksts katrai saturiskajai sadaļai;
- responsīvs dizains datoram, planšetei un telefonam;
- gaišais un tumšais režīms;
- frontend CRUD plānotājs bez datubāzes, izmantojot `localStorage`;
- attēlu vietturi, kurus vēlāk var aizstāt ar autora bildēm.

## GitHub Pages publicēšana

1. Izveido GitHub repozitoriju un augšupielādē šos failus.
2. GitHub repozitorijā atver `Settings`.
3. Atver `Pages`.
4. Pie `Build and deployment` izvēlies `Deploy from a branch`.
5. Pie `Branch` izvēlies `main` un mapi `/root`.
6. Saglabā. Pēc dažām minūtēm GitHub parādīs publisko saiti.

## Kas jānomaina pirms nodošanas

- Attēlu vietturus aizstāj ar savām bildēm, kad tās ir gatavas.
